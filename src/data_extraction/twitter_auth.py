...
def find_keyfile(directory=".", key_id=0):
    """
    ...
    for root, dirnames, filenames in os.walk(directory):
        for filename in filenames:
            if filename == f'keys_{key_id}.yaml':
                yield os.path.join(root, filename)


def authorize(filepath=None, key_id=0, directory=None):
    """
    ...
    try:
    	if directory:
    	     filepath = filepath or next(find_keyfile(directory=directory, key_id= key_id))	
    	else:
	     filepath = filepath or next(find_keyfile(key_id= key_id))
    ...
    auth_object = OAuth1Session(client_key=keys["consumer_key"],
